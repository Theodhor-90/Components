...
| 3   | `packages/ui/src/components/tooltip/tooltip.tsx`         | Create | Implementation: re-exports for Provider/Root/Trigger, styled TooltipContent with portal         |
...
- `@radix-ui/react-tooltip` — Radix Tooltip primitive (add to `packages/ui/package.json` under `dependencies`)
...
- `TooltipProvider` — direct re-export of `TooltipPrimitive.Provider` (as `const` assignment, matching Popover/Collapsible pattern)
- `Tooltip` — direct re-export of `TooltipPrimitive.Root` (as `const` assignment)
- `TooltipTrigger` — direct re-export of `TooltipPrimitive.Trigger` (as `const` assignment)
- `TooltipContent` — styled wrapper function component

**Key Logic**:

- `TooltipProvider`, `Tooltip`, and `TooltipTrigger` are assigned via `export const X = TooltipPrimitive.Y` — they are pure Radix re-exports with no custom `data-slot` (matching the pattern where Popover root is `export const Popover = PopoverPrimitive.Root` and Collapsible root is `export const Collapsible = CollapsiblePrimitive.Root`).
...
5. **`data-slot="tooltip-content"`** — after hover, verify content element has correct data-slot
6. **className merging on content** — pass custom className, verify it's applied alongside base styles
7. **Default `sideOffset`** — after hover, verify content has `data-side` attribute (proves positioning is active)
8. **Ref forwarding on TooltipContent** — use `createRef` and verify ref.current is an HTMLDivElement
9. **Accessibility** — hover to open, then `axe(document.body)` has no violations
...
**Action**: Add `@radix-ui/react-tooltip` to the `dependencies` object.

```json
"@radix-ui/react-tooltip": "^1.2.7"
```

Place it alphabetically after `@radix-ui/react-toggle-group` and before `@radix-ui/react-visually-hidden`.
...
| 5   | `data-slot` on content           | Attribute   | Hover to open, verify `document.querySelector('[data-slot="tooltip-content"]')` is in the document            |
| 6   | className merging on content     | Styling     | Pass `contentClassName="custom-class"`, hover to open, verify element has both `custom-class` and base styles |
| 7   | default sideOffset is active     | Positioning | Hover to open, verify content element has `data-side` attribute (Radix adds this for positioned content)      |
| 8   | ref forwarding on TooltipContent | Ref         | Pass a `createRef` to TooltipContent (requires separate test component), verify `ref.current` is an element   |
| 9   | no accessibility violations      | A11y        | Hover to open, `axe(document.body)` has no violations                                                         |

## 6. Implementation Order

1. **Install dependency** — Add `@radix-ui/react-tooltip` to `packages/ui/package.json` and run `pnpm install`
...
4. **Create `tooltip.tsx`** — Implement re-exports and styled `TooltipContent`
...
7. **Create `tooltip.test.tsx`** — Write all tests
