I have all the context needed. Now I can produce the complete implementation plan for the Avatar Group component (task t02).
...
| 6   | `packages/ui/src/index.ts`                                         | Modify | Add AvatarGroup, AvatarGroupProps, and style constant exports                              |
...
- No CVA — single visual treatment, static constants only (matching Table/Card pattern).
...
} from './components/avatar-group/avatar-group.styles.js';
```

This follows the established pattern: component + type from the `.tsx` file, style constants from the `.styles.ts` file.
...
6. **`packages/ui/src/index.ts`** — Add AvatarGroup exports. Depends on implementation and styles files.
...
- **Parent spec requires**: Descending z-index via `z-[N]` (implied by "descending `z-index` via inline `style={{ zIndex }}`" — the spec actually prescribes inline style).
- **Assessment**: The task spec explicitly prescribes inline `style={{ zIndex }}`, which is correct. Tailwind's z-index utilities (`z-10`, `z-20`, etc.) use preset values, and arbitrary value syntax (`z-[N]`) cannot accept runtime-computed values in JSX. Inline style is the only viable approach for dynamic z-index values and is consistent with how Radix UI components handle dynamic positioning.
- **Deviation**: None — the task spec already prescribes the correct approach. However, this is noted because the root AGENTS.md states "Never use inline styles instead of Tailwind classes." This is a valid exception: z-index values are dynamically computed from child count and cannot be expressed as static Tailwind classes. The `-space-x-3` Tailwind utility handles the static overlap, while only the dynamic z-index uses inline style.

None (beyond the z-index note above, which aligns with the task spec).
