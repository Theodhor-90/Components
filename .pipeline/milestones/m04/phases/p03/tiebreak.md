Good. Now I have all the context I need. Let me produce the definitive specification.
...
### DD-3: Search Input uses `useControllableState` from `@components/hooks`

Search Input uses the `useControllableState` hook exported from `@components/hooks` — the same hook used by other form controls in the library. The hook accepts `prop` (controlled value), `defaultProp` (uncontrolled initial value), and `onChange` parameters, and returns a `[value, setValue]` tuple. When `value` is provided, the component is controlled; otherwise it tracks state internally. The `onClear` callback uses `setValue('')` to clear in both modes.

### DD-4: No CVA variants for Empty State

Empty State has no visual variants (no size, color, or style variants). Its styles are exported as static string constants from `empty-state.styles.ts`. This avoids unnecessary CVA overhead for a single-variant component.

### DD-5: Search Input uses static style strings alongside imported `inputVariants`

Search Input's `<input>` element reuses `inputVariants` from the Input component. The container `<div>`, icon, and clear button use static style strings. No new CVA variant definition is needed in `search-input.styles.ts` — the file exports static string constants for the container, icon, and clear button.

### DD-6: No new npm dependencies

Both components are custom and use only native HTML elements. No Radix primitives or third-party libraries are required. The only cross-package dependency beyond `@components/utils` is `@components/hooks` (for `useControllableState`), which is already a workspace dependency of `@components/ui`. The only cross-component dependency is the import of `inputVariants` from the Input component's styles file.
...
- **`empty-state.tsx`** — Implementation:
  ...
  - Conditionally renders icon wrapped in `<div data-slot="empty-state-icon">` with `emptyStateIconStyles`
  - Renders title in `<h3 data-slot="empty-state-title">` with `emptyStateTitleStyles`
  - Conditionally renders description in `<p data-slot="empty-state-description">` with `emptyStateDescriptionStyles`
  - Conditionally renders action wrapped in `<div data-slot="empty-state-action">` with `emptyStateActionStyles`
...
- **`search-input.tsx`** — Implementation:
  ...
  - Uses `useControllableState` from `@components/hooks` with `{ prop: value, defaultProp: defaultValue, onChange: onChangeHandler }` where `onChangeHandler` extracts the string value from the change event and passes it through
  - Renders clear button (`<button type="button">` with inline SVG X icon) with `searchInputClearStyles`, visible only when input has a value
  - On Enter keypress: calls `onSearch` with current value
  - On clear button click: calls `setValue('')` to clear, calls `onClear`, refocuses the input via a ref
...
8. Search Input supports both controlled (`value`/`onChange`) and uncontrolled usage via `useControllableState`
...
- `@components/hooks` — provides `useControllableState` (already a workspace dependency of `@components/ui`)
...
- None — both components are custom and use only existing workspace and native dependencies
