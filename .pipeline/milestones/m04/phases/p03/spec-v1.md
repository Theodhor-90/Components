...
- **`search-input.types.ts`** — Define `SearchInputProps` extending `React.ComponentProps<'input'>` (omitting `type`) with:
  - `onSearch?: (value: string) => void` — fired on Enter keypress
  - `onClear?: () => void` — fired when clear button is clicked
  - `value?: string` — controlled value
  - `onChange?: React.ChangeEventHandler<HTMLInputElement>` — controlled change handler
