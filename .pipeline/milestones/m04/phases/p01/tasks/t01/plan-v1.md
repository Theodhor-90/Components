No problem — I have enough information from the specs and the existing codebase. The key observation is that Card (the compound component reference) does NOT use `asChild`/Slot. The task spec requires `asChild` on all Table sub-components, so I'll follow the task spec since it explicitly calls for it.
...
| `tableFooterStyles`  | `'border-t bg-muted/50 font-medium [&>tr]:last:border-b-0'`                                          |

These match the shadcn/ui Table styles. The `[&:has([role=checkbox])]` selectors allow tighter spacing when cells contain checkboxes (common in data tables).
...
4. **Base styling tests** (8 tests — one per sub-component):
   ...
   - TableHeader and TableBody: assert the element is rendered (their styles use child selectors `[&_tr]:border-b` which are not applied as classes on the element itself, so we verify the element renders correctly).
...
7. **asChild rendering test** (1 test):
   - Renders `<TableRow asChild><div data-testid="custom-row">Custom</div></TableRow>` (wrapped in valid table context), asserts the custom element receives the `data-slot` and className from TableRow.
...
| 19  | `TableRow applies base styling`                        | Styling     | `border-b`, `transition-colors` classes                                             |
| 20  | `TableHead applies base styling`                       | Styling     | `h-12`, `px-4`, `text-left`, `align-middle`, `font-medium`, `text-muted-foreground` |
| 21  | `TableCell applies base styling`                       | Styling     | `p-4`, `align-middle` classes                                                       |
| 22  | `TableCaption applies base styling`                    | Styling     | `mt-4`, `text-sm`, `text-muted-foreground`                                          |
| 23  | `TableFooter applies base styling`                     | Styling     | `border-t`, `font-medium` classes                                                   |
| 24  | `each sub-component merges custom className`           | className   | All sub-components have both `custom-class` and base classes                        |
| 25  | `Table forwards ref`                                   | Ref         | `ref.current` is `HTMLTableElement` with `data-slot="table"`                        |
| 26  | `TableRow supports asChild`                            | asChild     | Custom element receives data-slot and className                                     |
| 27  | `Table wraps table in overflow container`              | Structure   | Parent div of `<table>` has `overflow-auto` class                                   |
| 28  | `renders semantic HTML structure`                      | Structure   | `table > thead > tr > th` and `table > tbody > tr > td` exist                       |
| 29  | `fully composed table has no accessibility violations` | a11y        | `axe(container)` returns no violations                                              |
...
**Deviation 2: Style string exports for TableHeader/TableBody use child selectors**
