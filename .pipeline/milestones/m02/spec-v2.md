Now I have all the context. Let me carefully address each feedback issue and produce the revised specification.
...
- **10 components** implemented as shadcn/ui ports adapted to the 5-file pattern (`.tsx`, `.styles.ts`, `.types.ts`, `.test.tsx`, `.stories.tsx`)
...
- `@hookform/resolvers` as a required dependency for the Form component — not listed in the master plan's tech stack table, but this package is the standard bridge between `react-hook-form` and `zod` that shadcn/ui's Form pattern depends on (`zodResolver` is imported from it). Without it, zod schemas cannot be connected to react-hook-form.
...
**Textarea** — shadcn/ui port. Styled `<textarea>` element with consistent styling matching Input. Supports an optional `autoResize` boolean prop (default `false`). When `autoResize` is `true`, the textarea uses the CSS `field-sizing: content` property to grow with its content — no JavaScript measurement required. A `min-h-[80px]` constraint prevents collapse when empty, and `max-h-[300px]` with `overflow-y: auto` prevents unbounded growth. Supports `aria-invalid` for error state, `disabled` state, and controlled/uncontrolled usage.
...
**Third-party dependencies**: `react-hook-form`, `zod`, `@hookform/resolvers` (provides `zodResolver` — the standard adapter that connects zod schemas to react-hook-form's validation interface, required by the shadcn/ui Form pattern).
...
6. Textarea `autoResize` prop enables CSS-based content-driven sizing
...
2. **Select positioning edge cases** — Radix Select uses its own built-in positioning engine. Content may overflow viewport or clip inside scroll containers. Mitigation: test in Storybook with constrained containers and verify scroll behavior.
...
6. **CSS `field-sizing: content` browser support** — The `field-sizing` property is relatively new. If browser support is insufficient for target consumers, the Textarea `autoResize` implementation may need a JavaScript fallback (measuring `scrollHeight` on input). Mitigation: verify support in target browsers before implementation; if needed, fall back to a `useEffect` that sets `style.height` on change.
