Now I have all the context. The feedback has exactly one issue: remove `selectScrollButtonVariants` from the `index.ts` export block since the scroll buttons are internal-only components. Let me produce the revised plan.
...
| 6   | `packages/ui/src/index.ts`                             | Modify | Add exports for `Select`, `SelectGroup`, `SelectValue`, `SelectTrigger`, `SelectContent`, `SelectItem`, `SelectLabel`, `SelectSeparator`, all corresponding type exports, and public CVA variant exports (excluding internal `selectScrollButtonVariants`) |
...
Note: `selectScrollButtonVariants` is defined in this file for use by the internal `SelectScrollUpButton` and `SelectScrollDownButton` components, but it is **not** exported from `packages/ui/src/index.ts` since the scroll button components are internal to `SelectContent` and not part of the public API.
...
3. **`SelectValue`** — Direct re-assignment of `SelectPrimitive.Value`. Exported as a component for consumers who need direct control over value rendering within `SelectTrigger`. While the phase spec (DD-5) states `SelectValue` is not exported, the shadcn/ui reference implementation exports it, and consumer apps may need it for custom trigger layouts. This is noted as a deviation in Section 8.
...
Note: `selectScrollButtonVariants` is intentionally **excluded** from the `index.ts` styles export. `SelectScrollUpButton` and `SelectScrollDownButton` are internal implementation details of `SelectContent` (per DD-5 of the phase spec) and are not exported as public API components. Exporting their CVA variant function would add unnecessary public API surface that no consumer can use. The `selectScrollButtonVariants` export remains available within the `select.styles.ts` module for internal use by `select.tsx` only.
...
6. **`packages/ui/src/index.ts`** — Add all public exports (components, types, CVA variants — excluding `selectScrollButtonVariants`).
