Good — same pattern. Only root variants exported. Now I have everything I need to write the revised plan.
...
| `packages/ui/src/index.ts`                             | Modify | Add Slider component, type, and root CVA variant export                                      |
...
- `sliderVariants` — base classes for the Root element (public API export)
- `sliderTrackVariants` — base classes for Track (internal use only)
- `sliderRangeVariants` — base classes for Range / filled portion (internal use only)
- `sliderThumbVariants` — base classes for Thumb(s) (internal use only)

No configurable variants (no `variant` or `size` props) — base classes only per DD-1 in the phase spec. Follows the same pattern as `switchVariants` / `switchThumbVariants` where sub-element variants exist in the styles file but are not re-exported from the public API.
...
2. Destructure `className`, `defaultValue`, `value`, `ref`, and spread `...props` from `SliderProps`
...
5. **Thumb rendering**: Compute the number of thumbs from `value ?? defaultValue ?? [0]` — map over the values array to render one `SliderPrimitive.Thumb` per value. This allows single-value (one thumb) and range (two thumbs) modes. Each thumb gets `cn(sliderThumbVariants())`.
6. React 19 `ref` as prop on Root — no `forwardRef`
7. **Controlled vs. uncontrolled prop forwarding**: Only pass `defaultValue` to `SliderPrimitive.Root` when `value` is not provided. When `value` is provided (controlled mode), pass `value` instead. This avoids sending both `value` and `defaultValue` to the Radix primitive simultaneously, keeping the controlled/uncontrolled contract clean.
...
**Note on controlled/uncontrolled handling**: `defaultValue` is destructured with a default of `[0]` to guarantee `thumbCount` is always a valid `number[]` for mapping. When `value` is provided (controlled mode), only `value` is forwarded to the Radix Root; when `value` is `undefined` (uncontrolled mode), only `defaultValue` is forwarded. This uses a conditional spread (`{...(value !== undefined ? { value } : { defaultValue })}`) to ensure clean prop separation. The remaining `...props` still includes `onValueChange` and other non-conflicting props.

### 3.4 `slider.test.tsx`

**Purpose**: Comprehensive test suite covering smoke rendering, ARIA attributes, controlled/uncontrolled modes, range mode, disabled state, keyboard interaction, ref forwarding, and accessibility.
...
11. **Keyboard interaction** — focus thumb, press `ArrowRight`, assert `aria-valuenow` increases by step amount; press `ArrowLeft`, assert value decreases by step amount
12. **Ref forwarding** — `createRef` receives the DOM element
13. **Accessibility (single)** — `axe` has no violations on single-thumb slider
14. **Accessibility (range)** — `axe` has no violations on range slider
15. **Accessibility (with label)** — `axe` has no violations when composed with Label via `aria-label`
...
**Keyboard interaction test detail** (test #11):
...
it('adjusts value on arrow key press', async () => {
  ...
  await user.click(thumb); // focus the thumb
...
**Add exports** at the end of the file. Only `sliderVariants` (the root CVA) is exported from the public API — `sliderTrackVariants`, `sliderRangeVariants`, and `sliderThumbVariants` are internal sub-element styles and remain private to `slider.styles.ts`, matching the Switch precedent where only `switchVariants` is publicly exported (not `switchThumbVariants`).
...
| 11  | adjusts value on arrow key press        | `render(<Slider defaultValue={[50]} step={1} aria-label="Test" />)`                   | Focus thumb, press `ArrowRight`, then `ArrowLeft` | After `ArrowRight`: `aria-valuenow="51"`. After `ArrowLeft`: `aria-valuenow="50"` |
| 12  | forwards ref                            | `render(<Slider ref={ref} aria-label="Test" />)`                                      | —                                                 | `ref.current` is an instance of `HTMLSpanElement`                                 |
| 13  | has no a11y violations (single)         | `render(<Slider defaultValue={[50]} aria-label="Slider" />)`                          | `axe(container)`                                  | No violations                                                                     |
| 14  | has no a11y violations (range)          | `render(<Slider defaultValue={[25, 75]} aria-label="Range" />)`                       | `axe(container)`                                  | No violations                                                                     |
| 15  | has no a11y violations (with label)     | Render with Label + `htmlFor`                                                         | `axe(container)`                                  | No violations                                                                     |
...
**Problem**: When neither `value` nor `defaultValue` is provided, the computed `thumbCount` variable would be `undefined`, and mapping would fail. The Radix Slider primitive internally defaults to `[0]` but does not expose its internal state for rendering.

**Alternative chosen**: Default `defaultValue` to `[0]` in the function parameter destructuring: `defaultValue = [0]`. This ensures the thumb mapping always has an array to iterate over. The `thumbCount` computation becomes `const thumbCount = value ?? defaultValue`, which is always a valid `number[]`. This matches the shadcn/ui approach where the component ensures a fallback value for thumb rendering.
...
### Deviation 3: Conditional `value`/`defaultValue` forwarding

**Phase spec code template shows**: Unconditionally passing `defaultValue={defaultValue}` to `SliderPrimitive.Root`.

**Problem**: When the consumer provides a `value` prop (controlled mode), unconditionally passing `defaultValue` would send both `value` and `defaultValue` to the Radix primitive simultaneously. While Radix handles this gracefully (controlled `value` takes precedence), it is semantically incorrect — a component should be either controlled or uncontrolled, not both. Passing both props is a React anti-pattern that could produce console warnings in development.

**Alternative chosen**: Use a conditional spread to forward only the relevant prop: `{...(value !== undefined ? { value } : { defaultValue })}`. When `value` is provided, only `value` is forwarded; when `value` is `undefined`, only `defaultValue` is forwarded. This keeps the controlled/uncontrolled contract clean and avoids any ambiguity.
