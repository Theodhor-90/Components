I now have all the context I need to produce a comprehensive phase specification. Here it is:
...
3. **Controlled and uncontrolled usage via `useControllableState`.** The Combobox follows the same controlled/uncontrolled pattern as DatePicker and TimePicker, using the `useControllableState` hook from `@components/hooks`. Single mode uses `value`/`defaultValue`/`onValueChange` with `string | undefined`. Multiple mode uses the same props with `string[]`.
...
6. **Grouping not supported in v1.** The initial implementation does not support option groups. If needed in the future, a `groups` prop can be added without breaking changes. This keeps the initial API surface minimal and focused.

7. **Trigger displays selected value(s).** In single mode, the trigger button shows the selected option's label (or placeholder text). In multiple mode, it shows a count badge (e.g., "3 selected") when more than one item is selected, or the single label when exactly one is selected.

### Color Picker

8. **Hardcoded Tailwind palette at shade 500.** The preset color grid uses the 22 Tailwind color families (slate, gray, zinc, neutral, stone, red, orange, amber, yellow, lime, green, emerald, teal, cyan, sky, blue, indigo, violet, purple, fuchsia, pink, rose) at shade 500. These are static hex values, not dynamic theme tokens, because the Color Picker is for selecting arbitrary colors, not theme-aware semantic colors.

9. **Hex input with `#` prefix.** The text input accepts a 6-character hex color string. The `#` prefix is shown as a visual label outside the input (not part of the input value) to prevent formatting confusion. The input validates against `/^[0-9a-fA-F]{0,6}$/` on each keystroke, rejecting invalid characters.

10. **Live preview swatch.** A colored square swatch next to the hex input updates in real time via inline `backgroundColor` style. This is the one exception to the "no inline styles" rule — dynamic user-selected colors cannot be expressed as Tailwind classes.

11. **Controlled and uncontrolled via `useControllableState`.** Same pattern as all other composed inputs. Props: `value`/`defaultValue`/`onValueChange` with `string` (hex color including `#` prefix, e.g., `"#ef4444"`).

12. **No external dependencies.** Color Picker uses Popover (M1), Button (M0), and Input (M2) — all already available. No color-manipulation library is needed since the component only handles hex strings and preset swatches.

### Shared

13. **No icon library.** Consistent with the rest of the project, all icons (chevron, check, palette, search, X) use inline SVGs with `fill="none"`, `stroke="currentColor"`, `strokeWidth="2"`, `strokeLinecap="round"`, `strokeLinejoin="round"`, `aria-hidden="true"`.

14. **Static styles in `.styles.ts` files.** Following the pattern established by Date Picker, Time Picker, Calendar, and Command, both components use plain string constants in their `.styles.ts` files rather than CVA variant functions, since neither component has meaningful visual variants requiring CVA's `cva()` utility.
...
- `combobox.types.ts` — `ComboboxProps` type with `options`, `value`, `defaultValue`, `onValueChange`, `placeholder`, `searchPlaceholder`, `emptyMessage`, `disabled`, `className`, `ref` props. `ComboboxOption` type with `value`, `label`, `disabled?` fields.
- `combobox.styles.ts` — Style constants for trigger button, popover content, selected item check icon, and placeholder text
- `combobox.tsx` — Single-select Combobox composing `Popover` + `PopoverTrigger` + `PopoverContent` + `Command` + `CommandInput` + `CommandList` + `CommandEmpty` + `CommandGroup` + `CommandItem`. Uses `useControllableState` for value management. Selecting an item closes the popover. Trigger button shows selected option label or placeholder.
- `combobox.test.tsx` — Tests: smoke render, renders placeholder when no value, opens popover on trigger click, filters options as user types in CommandInput, selects an option and closes popover, displays selected option label in trigger, controlled mode works, uncontrolled mode works, disabled state prevents opening, vitest-axe accessibility check
...
- When `mode="multiple"`: value type becomes `string[]`, selecting an item toggles it (popover stays open), trigger shows count badge or single label, each selected item shows a check icon
...
- Update `combobox.test.tsx` — Additional tests: multi-select toggles items, multi-select trigger shows count, create-option appears for unmatched input, create-option callback fires, create-option hidden when all options match
...
- `color-picker.types.ts` — `ColorPickerProps` type with `value`, `defaultValue`, `onValueChange`, `disabled`, `placeholder`, `className`, `ref` props. All color values are hex strings including `#` prefix (e.g., `"#ef4444"`).
...
- `color-picker.tsx` — Color Picker composing `Popover` + `PopoverTrigger` + `PopoverContent` + `Button` (trigger) + `Input` (hex). Trigger shows a small color swatch preview and hex value (or placeholder). Popover contains: (1) a grid of 22 preset color swatches, (2) a hex input with `#` label, (3) a preview swatch showing the current color in real time. Uses `useControllableState` for value management. Clicking a preset swatch sets the value. Typing in the hex input updates the value in real time. Hex input validates against `/^[0-9a-fA-F]{0,6}$/`.
- `color-picker.test.tsx` — Tests: smoke render, renders placeholder when no value, opens popover on trigger click, clicking preset swatch sets value, hex input updates value in real time, hex input rejects invalid characters, preview swatch reflects current value, trigger shows selected color swatch, controlled mode works, uncontrolled mode works, disabled state prevents opening, vitest-axe accessibility check
...
9. `pnpm typecheck` passes with no TypeScript errors across the entire monorepo
10. `pnpm test` passes with zero failures, including vitest-axe accessibility assertions for both components
11. `pnpm lint` passes with no errors in new files
12. Both components render correctly in Storybook with all stories and autodocs tags
13. `Combobox`, `ComboboxProps`, `ComboboxOption`, `ColorPicker`, and `ColorPickerProps` are all exported from `packages/ui/src/index.ts`
...
- `useControllableState` from `@components/hooks` (used by both components for controlled/uncontrolled state management)
