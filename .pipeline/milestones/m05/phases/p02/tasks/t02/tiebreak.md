Now I have all the context needed. Let me produce the definitive plan.

# Task Plan: Calendar Component
...
- No CVA `VariantProps` — Calendar has a single visual presentation with no variant axes

### 3.2 `calendar.styles.ts`

Exports named string constants (not CVA functions) for each DayPicker v9 `classNames` key. This follows the same pattern used by `popover.styles.ts` for components without visual variants.
...
| `calendarButtonPreviousStyles` | `button_previous` | `absolute left-1 inline-flex items-center justify-center rounded-md border border-input bg-background h-7 w-7 hover:bg-accent hover:text-accent-foreground` — left nav button matching Button outline/icon pattern |
| `calendarButtonNextStyles` | `button_next` | `absolute right-1 inline-flex items-center justify-center rounded-md border border-input bg-background h-7 w-7 hover:bg-accent hover:text-accent-foreground` — right nav button matching Button outline/icon pattern |
| `calendarMonthGridStyles` | `month_grid` | `w-full border-collapse space-y-1` — table element |
| `calendarWeekdaysStyles` | `weekdays` | (row of day-of-week headers) |
...
| `calendarDayButtonStyles` | `day_button` | `inline-flex items-center justify-center h-9 w-9 rounded-md font-normal hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring` — clickable day button |
| `calendarSelectedStyles` | `selected` | `bg-primary text-primary-foreground hover:bg-primary/90` — selected day |
...
| `calendarRangeStartStyles` | `range_start` | `rounded-l-md` — range start left-rounded corners |
| `calendarRangeEndStyles` | `range_end` | `rounded-r-md` — range end right-rounded corners |
...
| `calendarFocusedStyles` | `focused` | `ring-1 ring-ring` — focused day ring styling |
...
5. **Chevron component** — Inline functional component rendering SVG arrows based on `orientation` prop. Uses the same `width="16" height="16" viewBox="0 0 24 24"` pattern established in `pagination.tsx` and `breadcrumb.tsx`.
6. **No `showOutsideDays` default** — consumers opt in via the standard react-day-picker `showOutsideDays` prop.
...
8. **`focused` classNames key is included** — wired to `calendarFocusedStyles` (`ring-1 ring-ring`) to style the focused day element with the project's standard ring token.
...
| 9 | Disabled dates are not selectable | `render(<Calendar mode="single" disabled={[new Date()]} />)` — verify the disabled day button has `disabled` attribute or appropriate styling and does not fire `onSelect` on click |
| 10 | Outside-month dates render with muted styling | `render(<Calendar showOutsideDays />)` — find an outside-month day cell and assert it has the `text-muted-foreground opacity-50` classes |
| 11 | Today's date has distinct styling | `render(<Calendar />)` — find today's date cell and assert it has the `bg-accent text-accent-foreground` classes |
| 12 | Custom className | `render(<Calendar className="custom-class" />)` — verify root element includes `custom-class` |
| 13 | Accessibility (vitest-axe) | `const { container } = render(<Calendar />)` — `expect(await axe(container)).toHaveNoViolations()` |
...
Append the following export:

```typescript
export { Calendar, type CalendarProps } from './components/calendar/calendar.js';
```

No styles export needed since Calendar uses string constants (not CVA variant functions that consumers would compose with).
...
10. All classNames use the react-day-picker v9 API keys (`month_caption`, `caption_label`, `button_previous`, `button_next`, `month_grid`, `day_button`, `focused`, etc.)
...
**Justification**: Calendar wraps `react-day-picker`'s `DayPicker` component, not a native HTML element. The `asChild` + Radix `Slot` pattern is designed for polymorphic rendering of leaf elements (e.g., rendering a Button as an `<a>` tag). It is not applicable to a complex third-party calendar widget. This is consistent with how other third-party wrapper components handle non-native primitives.
...
**Justification**: Since there are no CVA variant functions (see Deviation 1), there are no variant exports for consumers to compose. The internal string constants are implementation details, not part of the public API. The `popover.styles.ts` pattern confirms this — `popoverContentStyles` is not exported from `index.ts`.