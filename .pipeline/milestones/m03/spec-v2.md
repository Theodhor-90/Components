Good, I have all the context. Now let me produce the revised specification addressing every feedback issue:

1. **Remove residual LLM text** (line 1 of v1)
2. **Remove Risks section** (scope creep)
3. **Remove Tabs keyboard details from exit criteria** (scope creep)
4. **Remove Breadcrumb-specific exit criterion** (scope creep)
5. **Remove Header-specific exit criterion** (scope creep)
6. **Specify breakpoint for App Layout responsive behavior** (ambiguity)
7. **Clarify Header user info area** (ambiguity — match master plan: always-present slot)
8. **Specify Sidebar keyboard shortcut** (ambiguity — Cmd+B / Ctrl+B)
9. **Simplify Dependency #1** (correctness)
...
- **Header** — custom component providing a top bar layout with a title slot (left), action button slots (right), and a user info area. Uses semantic `<header>` element with `data-slot="header"`. All three slot areas are always rendered; consumers pass content via props/children.
- **App Layout** — custom component that composes Sidebar + Header + a scrollable main content area. On viewports below Tailwind's `md` breakpoint (768px), the sidebar collapses automatically. Provides a consistent layout structure across consumer apps.
...
1. **Milestone 2 complete** — prerequisite chain (M1 → M2 → M3); M2 is already complete
...
6. **Sidebar CSS tokens** — the `sidebar-*` custom properties already exist in `globals.css` (verified: `--sidebar-background`, `--sidebar-foreground`, `--sidebar-primary`, `--sidebar-primary-foreground`, `--sidebar-accent`, `--sidebar-accent-foreground`, `--sidebar-border`, `--sidebar-ring` are defined for both light and dark themes)
