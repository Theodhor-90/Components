I now have complete context on all patterns. Here is the specification:
...
| `packages/ui/src/index.ts`                             | Modified — add `Header`, `type HeaderProps`, `headerVariants` exports                                                                                  |
...
8. **`renders as child element when asChild is true`** — Render `<Header asChild><div data-testid="custom">Content</div></Header>`. Assert the custom div has `data-slot="header"` and the root is a `<div>`, not a `<header>`.

9. **`has no accessibility violations`** — Render `<Header>App Title</Header>`, run `axe()`, assert no violations.

**Additional test:** `merges custom className` — Render with `className="custom-class"`, assert the root element has both `custom-class` and the base CVA class (e.g., `bg-background`).
...
Add the following exports after the last existing export block (currently ScrollArea):
...
6. **`index.ts`** — Add `Header`, `type HeaderProps`, and `headerVariants` exports. This is the final integration step.
