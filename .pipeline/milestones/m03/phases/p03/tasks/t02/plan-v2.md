Now I have all the context needed. The feedback is clear and focused: the test plan is missing a test for controlled sidebar state (`open` + `onOpenChange`). Let me produce the revised plan.
  ...
  return (
    ...
    >
      {isDesktop ? (
        ...
        <Sheet open={open} onOpenChange={(sheetOpen) => { if (sheetOpen !== open) toggleSidebar(); }}>
...
| 8   | `supports controlled sidebar state`     | Render with `open={false}` and `onOpenChange={onOpenChangeSpy}` with desktop mock. Assert sidebar reflects the collapsed state (collapsed styling / `w-0`). Simulate a sidebar toggle interaction (e.g., click the `SidebarTrigger` rendered inside a test wrapper, or use `useSidebar().toggleSidebar()`). Assert `onOpenChangeSpy` was called with `true`. |
| 9   | `merges custom className`               | Render with `className="custom-class"`. Assert root element has both `custom-class` and the base flex classes.                                                                                                                                                                                                                                               |
| 10  | `has no accessibility violations`       | Render a full AppLayout with sidebar, header, and children. Run `axe()` on the container. Assert `toHaveNoViolations()`.                                                                                                                                                                                                                                     |

### Test #8 detail — controlled sidebar state

This test addresses the task spec verification criteria #3 ("Supports controlled `open` / `onOpenChange` and uncontrolled `defaultOpen` sidebar state") and the phase spec exit criteria #7. The test:

1. Creates a `vi.fn()` spy for `onOpenChange`.
2. Renders `<AppLayout open={false} onOpenChange={onOpenChangeSpy} sidebar={<nav>Nav</nav>}>Content</AppLayout>` with `useMediaQuery` returning `true` (desktop).
3. Asserts the sidebar is in the collapsed state — checks that the sidebar content element has the collapsed width styling (e.g., `w-0` or the SidebarProvider's collapsed data attribute).
4. Triggers a sidebar toggle — the simplest approach is to include a `SidebarTrigger` in the rendered content (e.g., pass it as part of the `header` prop: `header={<SidebarTrigger data-testid="toggle" />}`), then click it with `userEvent.click(screen.getByTestId('toggle'))`.
5. Asserts `onOpenChangeSpy` was called with `true`, confirming the controlled callback fires when sidebar state changes.
