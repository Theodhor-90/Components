...
All six Card sub-components (`Card`, `CardHeader`, `CardContent`, `CardFooter`, `CardTitle`, `CardDescription`) are defined in a single `card.tsx` file and share a single `card.styles.ts` file. Each sub-component has its own CVA definition (or plain class string) within that styles file. The types file exports a union or individual prop types for each sub-component. This matches the shadcn/ui pattern where compound components are co-located.
...
### DD-6: `asChild` only on leaf components

Following the shadcn/ui convention, `asChild` (via Radix `Slot`) is supported on single-element components (Separator, Badge, Skeleton, Spinner) but not on compound component wrappers (Card, Alert). Sub-components like `CardTitle` support `asChild` since they are leaf elements.

### DD-7: Skeleton has no CVA variants

Skeleton is intentionally variant-free. Consumers control its dimensions entirely through `className` (e.g., `className="h-4 w-[200px]"`). The component only applies base styling: `animate-pulse`, `rounded-md`, and `bg-muted`. This matches the shadcn/ui Skeleton API.
...
- Create `packages/ui/src/components/card/` directory with 5 files:
  - `card.tsx` — Six named exports: `Card` (`<div>`, `data-slot="card"`), `CardHeader` (`<div>`, `data-slot="card-header"`), `CardTitle` (`<div>` with `asChild`, `data-slot="card-title"`), `CardDescription` (`<div>`, `data-slot="card-description"`), `CardContent` (`<div>`, `data-slot="card-content"`), `CardFooter` (`<div>`, `data-slot="card-footer"`). Card root applies `rounded-xl border bg-card text-card-foreground shadow-sm`. Re-exports all types.
...
- Export all six components, all six prop types from `packages/ui/src/index.ts`
...
- Create `packages/ui/src/components/spinner/` directory with 5 files:
  - `spinner.tsx` — Renders an animated SVG with a circular path. Props: `size` (`"sm" | "md" | "lg"`), `className`, `ref`. Applies `data-slot="spinner"`, `role="status"`. Includes a `<VisuallyHidden>` or `<span className="sr-only">` element with default text `"Loading"` (overridable via `aria-label`). SVG uses `currentColor` for stroke so it inherits text color from parent.
...
- Create `packages/ui/src/components/alert/` directory with 5 files:
  - `alert.tsx` — Three named exports: `Alert` (`<div role="alert">`, `data-slot="alert"`), `AlertTitle` (`<h5>` with `asChild`, `data-slot="alert-title"`), `AlertDescription` (`<div>`, `data-slot="alert-description"`). Alert root supports `variant` prop (`"default" | "destructive"`).
...
This is Phase 1 of Milestone 1. It has no dependency on any other phase. All subsequent phases in Milestone 1 (Overlay Primitives, Accessibility Primitives) are independent of this phase and may be developed in parallel, though the patterns established here will inform their implementation.
