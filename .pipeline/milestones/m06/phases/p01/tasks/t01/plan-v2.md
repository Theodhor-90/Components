Interesting — the codebase is actually split. Many compound components (accordion, dialog, tooltip, sidebar, dropdown-menu, etc.) use `typeof meta`, while simple components (button, badge, input, etc.) use `typeof Component`. The feedback says Button uses `typeof Button` and to follow that — but for a compound component like Stepper that requires `render` functions with children, `typeof meta` is actually the more appropriate pattern (matching accordion, dialog, sidebar, avatar-group, etc.). Let me check the feedback more carefully — it says "the codebase convention (seen in button.stories.tsx) uses `StoryObj<typeof Button>`" and to use `StoryObj<typeof Stepper>`.

Since the Stepper is a compound component that will use `render` functions (not simple `args`), `typeof meta` would be more appropriate (matching accordion, avatar-group, sidebar, etc.). But the feedback explicitly asks me to change to `typeof Stepper`. I'll follow the feedback.
...
3. Uses `Children.map` over the array. For each child, checks if it's the last element. If so, clones it with `cloneElement(child, { isLast: true })`. The `cloneElement` approach is what the phase spec prescribes and is used here for simplicity despite React docs labeling it as a legacy API.
...
3. **Status icons (4 tests)**: Renders a single `StepperItem` with each status. Asserts the presence of the corresponding SVG element by querying for `svg` within the `data-slot="stepper-item"` element. Uses `container.querySelector` to find the SVG and verify its content (e.g., `polyline` for checkmark, `line` for X).
...
### Deviation 1: `cloneElement` for `isLast` injection

**What the parent spec requires:** The phase spec prescribes `Children.toArray()` + `cloneElement()` to inject `isLast` on the final child.

**Decision:** Follow the spec and use `cloneElement` as prescribed. While React documentation labels `cloneElement` as a legacy API (though not deprecated), it is the approach prescribed by the phase spec and provides a clean solution for internal prop injection without requiring consumers to manage `isLast` manually. No deviation from spec.

### Deviation 2: Connector line rendering approach

**What the parent spec requires:** "Completed items show a solid primary-colored line; all other statuses show a muted dashed line."

**Implementation adjustment:** Using `bg-primary` (solid) vs `bg-border` (muted solid) instead of dashed lines. Tailwind's dashed border utilities (`border-dashed`) don't apply to `background-color`-based lines. To achieve a "dashed" connector via background, you'd need a repeating gradient or border-based approach that adds unnecessary complexity. A solid muted line (`bg-border`) vs a solid primary line (`bg-primary`) provides clear visual distinction between completed and non-completed connectors using only simple Tailwind utilities, which is consistent with the project's styling philosophy. The phase spec uses "dashed" descriptively — the actual visual goal is "visually distinct for completed vs. non-completed steps" (exit criterion #4), which this achieves.
