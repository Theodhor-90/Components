I now have complete understanding of all patterns. Here is the full implementation plan:
...
- `stepperConnectorPendingStyles` — `'bg-muted-foreground/30'` (muted dashed line for non-completed)
...
3. Uses `Children.map` over the array. For each child, checks if it's the last element. If so, clones it with `cloneElement(child, { isLast: true })`. This mirrors the AvatarGroup pattern from this codebase.
...
7. **Connecting lines**: Rendered as a `<div>` with height/width depending on orientation. Line color determined by status:
   - `completed` → `stepperConnectorCompletedStyles` (solid primary)
   - All others → `stepperConnectorPendingStyles` (muted)
...
**Test setup:** Uses `@testing-library/react` for rendering/queries, `vitest` for assertions/mocking, `vitest-axe` for accessibility testing. Follows the same import pattern as `button.test.tsx` and `avatar-group.test.tsx`.
...
### Deviation 1: No `cloneElement` for `isLast` injection — using wrapper-based approach considered but rejected

**What the parent spec requires:** The phase spec prescribes `cloneElement()` to inject `isLast` on the final child.

**Assessment:** The `cloneElement` approach is technically sound and matches the AvatarGroup precedent. However, `cloneElement` is marked as a legacy API in React documentation (though not deprecated). The approach works correctly with the `StepperItemInternalProps` typing pattern.

**Decision:** Follow the spec and use `cloneElement` as prescribed. No deviation needed — this confirms the spec is correct.
