...
1. **Compound component with React Context for orientation.** Stepper manages orientation via a lightweight React Context, matching the Toggle Group pattern established in this codebase. The `Stepper` container accepts an `orientation` prop and provides it to children through `StepperContext`. `StepperItem` reads orientation from context, so consumers set `orientation` only once on the root `Stepper`. Individual `StepperItem` components do **not** accept an `orientation` prop. This mirrors `ToggleGroup` → `ToggleGroupItem` where `variant`/`size` flow through context.

2. **Orientation via CVA variants on the container.** The `Stepper` container uses a CVA variant for `orientation` (`horizontal` | `vertical`) that switches between `flex-row` and `flex-col` layout. `StepperItem` reads orientation from `StepperContext` to apply the correct internal layout (icon beside text for horizontal, icon above text for vertical).

3. **Last-item detection handled by the parent via `Children.toArray()`.** The `Stepper` container iterates over children using `React.Children.toArray()` and injects `isLast` via `React.cloneElement()` on the final child. This follows the AvatarGroup precedent in this codebase where the parent processes children as an array. `isLast` is **not** part of the public `StepperItemProps` type — it is declared as an internal-only prop in the types file (prefixed with a JSDoc `@internal` annotation) and is never set by consumers.

4. **Connecting lines are part of `StepperItem`.** Each `StepperItem` (except the last, detected via the injected `isLast` prop) renders a connecting line after its icon. The line's visual treatment is driven by the item's own status: `completed` items show a solid primary-colored line; all other statuses show a muted dashed line. This avoids a separate `StepperConnector` component and keeps the DOM structure flat within each item.

5. **Status icons are inline SVGs.** Rather than importing an icon library, status icons (circle, filled circle, checkmark, X) are small inline SVGs defined in the component file. This avoids adding a dependency and keeps the component self-contained. The icons are wrapped in a `<span>` with `aria-hidden="true"` since the status is conveyed through `aria-current="step"` on the active item and sr-only text.

6. **No `asChild` support.** Stepper and StepperItem render as semantic `<div>` elements. Polymorphic rendering is not useful for this component — consumers compose by nesting content inside `StepperItem`, not by replacing the root element.
...
3. **Last-item detection handled by the parent via `Children.toArray()`.** Same pattern as Stepper: the `Timeline` container iterates children, injects `isLast` via `cloneElement()` on the final child. `isLast` is internal-only and not part of the public `TimelineItemProps` type.

4. **Connecting line via `border-l` on a wrapper element.** Each `TimelineItem` (except the last, detected via the injected `isLast` prop) renders a wrapper `<div>` around its content area that carries `border-l-2 border-border`, producing a continuous vertical line connecting adjacent dots. The last item omits the border. This uses standard Tailwind utilities with no custom CSS or pseudo-elements.

5. **Three statuses with distinct visual mappings.** `TimelineItem` supports three status values, each with a visually unique dot color:
   - `default` → `bg-primary` (standard brand-colored dot)
   - `error` → `bg-destructive` (red dot)
   - `warning` → `bg-accent` (accent-colored dot)

   Three statuses with three distinct colors is preferred over four statuses where two would be visually identical.

6. **Consumer controls ordering.** The component renders items in the order provided. No internal sorting by timestamp or any other field.
...
- `packages/ui/src/components/stepper/stepper.tsx` — `StepperContext` (React Context providing `{ orientation }`), `Stepper` container that wraps children in `StepperContext.Provider` and injects `isLast` into the final child via `cloneElement()`, and `StepperItem` sub-component that reads orientation from context. Horizontal/vertical layout support and four status states (pending, active, completed, error). Connecting lines between items (omitted on last). `data-slot="stepper"` and `data-slot="stepper-item"` attributes.
- `packages/ui/src/components/stepper/stepper.types.ts` — `StepperContextType` with `orientation` field. `StepperProps` extending `React.ComponentProps<'div'>` with `orientation` variant prop (defaults to `'horizontal'`). `StepperItemProps` extending `React.ComponentProps<'div'>` with required `status` prop (`'pending' | 'active' | 'completed' | 'error'`), required `title` (string), optional `description` (string). Internal-only `StepperItemInternalProps` extending `StepperItemProps` with `isLast` (boolean), annotated with `@internal` JSDoc. CVA `VariantProps` integration.
- `packages/ui/src/components/stepper/stepper.styles.ts` — CVA definitions: `stepperVariants` (orientation: horizontal/vertical), `stepperItemVariants` (status-based icon and line coloring). Plain string exports for sub-element styles (title, description, connector line, icon container).
...
- `packages/ui/src/components/timeline/timeline.tsx` — `Timeline` container that injects `isLast` into the final child via `cloneElement()`, and `TimelineItem` sub-component. Vertical layout with continuous connecting line between dots using `border-l-2 border-border` on a content wrapper (omitted on last item). `data-slot="timeline"` and `data-slot="timeline-item"` attributes.
- `packages/ui/src/components/timeline/timeline.types.ts` — `TimelineProps` extending `React.ComponentProps<'div'>`. `TimelineItemProps` extending `React.ComponentProps<'div'>` with required `title` (string), optional `timestamp` (string), optional `status` (`'default' | 'error' | 'warning'`, defaults to `'default'`), and `children` for content body. Internal-only `TimelineItemInternalProps` extending `TimelineItemProps` with `isLast` (boolean), annotated with `@internal` JSDoc.
...
- Add Stepper exports to `packages/ui/src/index.ts`:
...
- Add Timeline exports to `packages/ui/src/index.ts`:
...
- Note: Internal types (`StepperItemInternalProps`, `TimelineItemInternalProps`) are **not** exported from `index.ts`.
...
7. Last-item detection is handled internally by both `Stepper` and `Timeline` containers via `Children.toArray()` + `cloneElement()` — consumers never pass `isLast`
8. `Timeline` renders items in a vertical sequence with a continuous connecting line (via `border-l-2 border-border`) between status dots
9. `TimelineItem` renders title (always), timestamp (when provided), content body (when provided via children), and a colored status dot
10. `timelineItemDotVariants` maps three statuses to three visually distinct dot colors: `default` → `bg-primary`, `error` → `bg-destructive`, `warning` → `bg-accent`
11. All public components, types, and variant functions are exported from `packages/ui/src/index.ts` with correct source files (components/types from `.js`, variants from `.styles.js`)
12. `pnpm typecheck` passes with zero errors
13. `pnpm test` passes with zero failures, including vitest-axe accessibility assertions for both components
14. Both components render in Storybook with autodocs and all stories display correctly
...
| `packages/ui/src/components/stepper/stepper.types.ts` | New file | Public and internal prop types, context type |
...
| `packages/ui/src/components/timeline/timeline.tsx` | New file | Timeline and TimelineItem implementations |
| `packages/ui/src/components/timeline/timeline.types.ts` | New file | Public and internal prop types |
