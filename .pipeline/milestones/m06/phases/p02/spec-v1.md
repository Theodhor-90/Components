Now I have a thorough understanding of the codebase patterns. Let me produce the specification document.
...
The 2-second "Copied!" feedback uses a `copied` boolean state toggled by `setTimeout`. The timeout is cleaned up on unmount via `useEffect` return to prevent state updates on unmounted components. No external animation library is used.

### DD-7: Code Block line numbers via CSS counter

When `showLineNumbers` is `true`, line numbers are rendered via an ordered list with CSS counters or explicit `<span>` elements per line, split from the `children` string. This avoids requiring consumers to pre-format their code content.
...
- **`copy-to-clipboard.types.ts`** — `CopyToClipboardProps` extending `React.ComponentProps<'button'>` with required `text: string` prop, optional `feedbackDuration?: number` (default 2000ms), and `asChild?: boolean` for polymorphic rendering via Radix Slot.
- **`copy-to-clipboard.styles.ts`** — `copyToClipboardVariants` CVA definition with size variants (`default`, `sm`, `icon`) mapping to Tailwind classes. Base classes include `inline-flex items-center justify-center gap-2` and cursor/transition utilities.
- **`copy-to-clipboard.tsx`** — Implementation using `@radix-ui/react-slot` for `asChild` support. Renders a `<button>` (or Slot) with `data-slot="copy-to-clipboard"`. Internal state tracks `copied` boolean. On click: calls `navigator.clipboard.writeText(text)`, sets `copied = true`, schedules `setTimeout` to reset after `feedbackDuration`. Renders a copy icon (inline SVG) that swaps to a checkmark icon (inline SVG) when `copied` is true. Includes `aria-label` that changes from "Copy to clipboard" to "Copied" for screen reader feedback. Cleanup via `useEffect` return clears the timeout on unmount.
- **`copy-to-clipboard.test.tsx`** — Tests: smoke render, `data-slot` attribute, ref forwarding, className merging, calls `navigator.clipboard.writeText` with correct text on click, icon swaps to checkmark after click, resets to copy icon after timeout (use `vi.useFakeTimers`), `asChild` polymorphic rendering, accessibility (vitest-axe). Mock `navigator.clipboard.writeText` as `vi.fn().mockResolvedValue(undefined)`.
- **`copy-to-clipboard.stories.tsx`** — Stories: Default, SmallSize, IconOnly, CustomFeedbackDuration, AsChild (rendered as an anchor), WithLongText. CSF3 with `tags: ['autodocs']`.
...
- **`connection-status.styles.ts`** — `connectionStatusVariants` CVA definition with a `status` variant. Base classes: `inline-flex items-center gap-2 text-sm`. Status variant maps to no extra classes on the container (color is on the dot). Separate exports: `connectionStatusDotVariants` CVA with `status` variant mapping to dot-specific classes — `connected`: `bg-green-500 dark:bg-green-400`, `connecting`: `bg-yellow-500 dark:bg-yellow-400 animate-pulse`, `disconnected`: `bg-red-500 dark:bg-red-400`. Base dot classes: `size-2 rounded-full shrink-0`.
...
- Add all three components, their sub-types, and CVA variant functions to `packages/ui/src/index.ts`:
  - `CopyToClipboard`, `type CopyToClipboardProps`, `copyToClipboardVariants`
...
- Also add missing Timeline exports from Phase 1 (`Timeline`, `TimelineItem`, `type TimelineProps`, `type TimelineItemProps`, `timelineVariants`, `timelineItemVariants`)
...
4. Copy to Clipboard calls `navigator.clipboard.writeText()` with the provided `text` on click, swaps to a checkmark icon, and resets after the feedback duration
...
10. Missing Timeline exports from Phase 1 are added to `packages/ui/src/index.ts`
...
| `packages/ui/src/components/copy-to-clipboard/copy-to-clipboard.styles.ts` | New | CVA variant definitions |
...
| `packages/ui/src/index.ts` | Modified | Add exports for all three components + missing Timeline exports |