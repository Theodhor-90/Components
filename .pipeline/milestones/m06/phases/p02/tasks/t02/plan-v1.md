I have all the context I need. Now I'll produce the complete implementation plan.
...
| 6 | `packages/ui/src/index.ts` | Modified | Add `CodeBlock` and `type CodeBlockProps` exports, plus style constant exports |
...
5. **Code block**: `<pre className={cn(codeBlockPreStyles)}>` wrapping a `<code>`:
   ...
   - When `showLineNumbers` is `true`: split `children` by `'\n'`, then map each line to a `<span>` row with `display: flex` (via `className="flex"`), containing:
     - `<span className={codeBlockLineNumberStyles}>{index + 1}</span>` — line number
     - `<span>{line}</span>` — code content (render empty lines as `'\n'` or a zero-width space to preserve height)
...
export {
  ...
  codeBlockHeaderStyles,
  codeBlockLineNumberStyles,
} from './components/code-block/code-block.styles.js';
```
...
<pre ...>
  <code>
    <span class="flex">
      <span class="select-none pr-4 text-right text-muted-foreground/50">1</span>
      <span>line 1</span>
    </span>
    <span class="flex">
      <span class="select-none pr-4 text-right text-muted-foreground/50">2</span>
      <span>line 2</span>
    </span>
  </code>
</pre>
...
6. **`packages/ui/src/index.ts`** — Add CodeBlock and style exports to the public API.
...
**Deviation 1: Line number gutter width**
...
- **Alternative chosen**: The line number `<span>` uses `inline-block` with a computed `min-width` based on the total line count's digit width (e.g., `style={{ minWidth: '2ch' }}` for 10–99 lines). This uses an inline `style` attribute solely for the dynamic `minWidth` calculation — all other styling uses Tailwind classes. This is the minimal approach to ensure line numbers align correctly regardless of digit count.
